...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE command = reg + 2;
    BYTE send[2] = { command, val };
    BYTE recv[2] = { 0, 0 };
    int returncode;

    // Select MAX3421E (assert chip select)
    XSpi_SetSlaveSelect(&SpiInstance, 0x01);
    returncode = XSpi_Transfer(&SpiInstance, send, recv, 2);

    // Deselect MAX3421E (deassert chip select)
    XSpi_SetSlaveSelect(&SpiInstance, 0x00);

    if (returncode != 0) {
        xil_printf("Error during Single host register write");
        return;
    }
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
    int returncode;
    BYTE command = reg + 2;  // Command byte with write bit set
    BYTE send[nbytes + 1];
    BYTE recv[nbytes + 1];
    int i;
    ...
    XSpi_SetSlaveSelect(&SpiInstance, 0x01);
    returncode = XSpi_Transfer(&SpiInstance, send, recv, nbytes + 1);
    XSpi_SetSlaveSelect(&SpiInstance, 0x00);

    if (returncode != 0) {
        xil_printf("Error during Multi-Byte write");
    }
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//return val
    int returncode;
    BYTE send[2] = { reg, 0 };  //second is a dummy byte.
    BYTE recv[2] = { 0, 0 };
    ...
    if (returncode != 0) {
        xil_printf("Error during Single host register read");
    }
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    // Build the send buffer: first byte is the register address; remaining bytes are dummy bytes.
    send[0] = reg;
    ...
    if (returncode != 0) {
        xil_printf("Error during Multi-byte register read");
    }

    // Copy received data (starting at recv[1]) into the provided data buffer.
    for (i = 0; i < nbytes; i++) {
        data[i] = recv[i + 1];
    }
